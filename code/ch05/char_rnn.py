...
def one_hot(indices, vocab_size):
    ...
    oh = torch.zeros(len(indices), vocab_size, device=DEVICE)
    oh.scatter_(1, indices.unsqueeze(1), 1.0)
    return oh


def make_sequences(text, c2i, seq_len):
    """Encode the full text then yield (input, target) index tensor pairs."""
...
class CharRNN(nn.Module):
    ...
    def forward(self, x_oh, hidden=None):
        """x_oh: (seq_len, vocab_size)"""
        out, hidden = self.rnn(x_oh.unsqueeze(1), hidden)  # unsqueeze batch=1
        logits = self.linear(out.squeeze(1))               # (seq_len, vocab_size)
...
def train(model, text, c2i, vocab_size):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x, y in make_sequences(text, c2i, SEQ_LEN):
            # detach hidden state from previous sequence graph
            hidden = hidden.detach()

            x_oh = one_hot(x, vocab_size)            # (seq_len, vocab_size)
            logits, hidden = model(x_oh, hidden)     # (seq_len, vocab_size)
...
def generate(model, c2i, i2c, vocab_size, seed="long long ago", length=500, temperature=0.8):
    ...
    # warm up on the seed
    for ch in seed[:-1]:
        idx = torch.tensor([c2i.get(ch, 0)], device=DEVICE)  # (1,)
        x_oh = one_hot(idx, vocab_size)                        # (1, vocab_size)
    ...
    for _ in range(length):
        ...
        x_oh = one_hot(idx, vocab_size)
        logits, hidden = model(x_oh, hidden)          # logits: (1, vocab_size)
