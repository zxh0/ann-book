"""
Character-level GRU on Tiny Shakespeare dataset.
Uses one-hot encoding and a GRU implemented in PyTorch.

Usage:
    python char_gru.py              # train + generate
    python char_gru.py --generate   # load checkpoint and generate only
"""
...
class CharGRU(nn.Module):
    def __init__(self, vocab_size, hidden_size, num_layers):
        ...
        # GRU 只有一个隐藏状态 h（没有 LSTM 的 cell state c）
        self.gru    = nn.GRU(vocab_size, hidden_size, num_layers)
        self.linear = nn.Linear(hidden_size, vocab_size)

    def forward(self, x_oh, hidden=None):
        """x_oh: (seq_len, batch, vocab_size)
        hidden: h，形状为 (num_layers, batch, hidden_size)
        """
        out, hidden = self.gru(x_oh, hidden)          # out: (seq, batch, hidden)
...
def main():
    ...
    model = CharGRU(vocab_size, HIDDEN_SIZE, NUM_LAYERS).to(DEVICE)
