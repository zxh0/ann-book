"""
Word-level GRU on Tiny Shakespeare dataset.
Uses Word2Vec embeddings (via gensim) and a GRU implemented in PyTorch.

Usage:
    python word_gru.py              # train word2vec + GRU, then generate
    python word_gru.py --generate   # load checkpoint and generate only
"""
...
def build_vocab(tokens):
    vocab = ["<unk>"] + sorted(set(tokens))   # index 0 is always <unk>
...
class WordGRU(nn.Module):
    ...
    def forward(self, x, hidden=None):
        ...
        out, hidden = self.gru(emb, hidden)   # out: (seq, batch, hidden)
...
def sample_logits(logits, temperature=0.8, top_k=0, top_p=0.0):
    ...
    # top-k: keep exactly the k highest-logit tokens (index-based, handles ties correctly)
    if top_k > 0:
...
def generate(model, t2i, i2t, vocab_size, seed="long long ago", length=200,
    ...
    unk_idx = t2i.get("<unk>", 0)

    # warm up on the seed
    for tok in seed_tokens[:-1]:
        idx = torch.tensor([[t2i.get(tok, unk_idx)]], device=DEVICE)  # (1, 1)
    ...
    for _ in range(length):
        idx = torch.tensor([[t2i.get(tok, unk_idx)]], device=DEVICE)
...
def main():
    ...
    model = WordGRU(vocab_size, EMBED_SIZE, HIDDEN_SIZE, NUM_LAYERS,
