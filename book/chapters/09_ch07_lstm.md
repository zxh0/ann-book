...
- 第一行：根据当前输入 $\mathbf{x}$ 和上一时刻的隐藏状态 $\mathbf{h}$ ，计算新的隐藏状态 $\mathbf{h}'$
- 第二行：根据新的隐藏状态 $\mathbf{h}'$ ，计算当前输出 $\mathbf{y}$
...
现在请特别关注上图左侧红框中的部分，也就是“根据输入和旧隐藏状态，计算新隐藏状态”的这一段逻辑。这一部分其实非常关键，因为：第一，它决定了“过去的信息如何影响现在”；第二，它是 RNN 能够处理序列的核心所在。如果我们把这部分计算抽象出来，并且暂时忽略内部的具体细节（权重、偏置、矩阵乘法等），就可以把它看作是一个“信息处理单元”。为了简化表示，我们可以将这个单元画成一个小梯形（或其他统一符号），表示它：接收输入 $\mathbf{x}$ 和历史状态 $\mathbf{h}$ ；输出新的状态 $\mathbf{h}'$ 。我们把这个抽象出来的结构，称为：门。
...
具体来说，LSTM 一共包含三个“真正的门”，以及一个用于生成候选信息的结构（通常称为“细胞候选”或“候选记忆”）。此外，它还引入了一个新的状态变量：**细胞状态（Cell State）**，它同样是一个向量，用粗体小写字母 $\mathbf{c}$ 表示。
...
与标准 RNN 相比，LSTM 最关键的变化是：除了隐藏状态 $\mathbf{h}$ ，还额外引入了细胞状态 $\mathbf{c}$ 。这两个状态的分工可以简单理解为： $\mathbf{h}$ ：短期状态（用于输出）； $\mathbf{c}$ ：长期记忆（用于跨时间传递信息）。也正是因为这种“短期 + 长期”的结构，LSTM 才得名 **长短期记忆网络**。
...
从上图中，以及公式，可以看到，LSTM 包含三个门和一个“细胞门”，它们的输入都是当前输入 $\mathbf{x}$ 和上一时刻的隐藏状态 $\mathbf{h}$ ：

- **遗忘门（Forget Gate）**：决定旧记忆 $\mathbf{c}$ 中哪些内容需要保留，对应上面公式第一行。遗忘门输出的是一个0～1直接的权重向量。
...
遗忘门的输出 $\mathbf{f}$ ，是一个元素取值在0～1之间的权重向量。它和记忆 $\mathbf{c}$ 进行逐元素相乘，这是它能决定忘掉那些东西的核心。如果 $\mathbf{f}$ 的某个元素解决于0，则模型倾向于忘记对应状态。否则，如果接近于1，模型倾向于保留对应状态。
...
和遗忘门类似，输入门的输出 $\mathbf{i}$ ，也是一个元素取值在0～1之间的权重向量。但是不同的是，我们还需要计算一个候选记忆 $\hat{\mathbf{c}}$ 。注意，由于使用了Tanh激活函数，隐藏候选记忆的每个元素，是-1～1，这个很重要（补充说明）。候选记忆是当前时刻新生成的、待筛选的临时记忆。候选记忆和输入门输出 $\mathbf{i}$ ，逐元素相乘，就得到了需要补充的记忆。
...
懂了遗忘门和输入门，输出门就相对好理解了。它基本上就是之前RNN层，只是略微改变了一下。差别在于，我们把记忆状态 $\mathbf{c}$ 也考虑了进来。输出门的计算逻辑如下图所示：
...
和遗忘门、输入门一样，输出门的输出 $\mathbf{o}$ ，也是元素取值在0～1之间的权重向量。和标准RNN模块不同，我们对 $\mathbf{c}$ 使用Tanh处理，然后和 $\mathbf{o}$ 逐元素相乘。输出门控制最终的短期记忆 $\mathbf{h}$ ，以及输出 $\mathbf{y}$ 。
...
GRU的主要简化：去掉细胞状态，重新回到单个隐藏状态 $\mathbf{h}$ 。三个门改成两个门。其中**重置门** （Reset Gate）：控制“过往历史要不要清空”。**更新门** （Update Gate）控制“旧记忆保留多少、新记忆写入多少”。下面是GRU的示意图：
