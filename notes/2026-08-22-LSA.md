...
以第一页为例，上面这个计算 $\mathbf{k}_n^{mean}$ 的公式，可以画成下面这样：
...
对于每一页（编号`p`），以及每一个indexer头（编号`j`），都可以根据上面高亮的这个求和公式算出来一个中间值。我们把这个中间值叫做 $I_{t,p,j}^{page}$ 。以第一页为例，这个计算过程可以表示成下面这样：

<img src="./images/lsa/hi3.png" alt="HI" style="zoom:50%;" />

把每个indexer头对应的 $I_{t,p,j}^{page}$ 加权求和，就得到了最终的打分。权重`w`和算token分数时是完全一样的。完整公式的计算过程，可以用下图表示：
...
在LSA论文里，页大小`P`固定取值128（也就是一页128个token），块大小`B`和`M`则属于超参数。论文指出，`B=8`（即一页16个块）、`M=1024`是比较合理的值。
