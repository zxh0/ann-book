...
我们再来看最后两个公式（46、47）。先合并在一起写，然后展开`v`那一项，并利用矩阵乘法结合律进行变换，最后就变成了下面这个样子：
...
> AI：MLA有两种计算方式。第一种，先恢复完整 K/V，再走标准 Attention，工程简单、兼容 FlashAttention，但 decode阶段成本较高。第二种，不恢复 K/V，而是在 latent 空间完成 Attention 的主要计算，decode 更快、更省算力，但由于计算图发生变化，需要专门实现 kernel。
