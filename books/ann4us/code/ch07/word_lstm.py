"""
Word-level LSTM on Tiny Shakespeare dataset.
Uses Word2Vec embeddings (via gensim) and an LSTM implemented in PyTorch.

Usage:
    python word_lstm.py              # train word2vec + LSTM, then generate
    python word_lstm.py --generate   # load checkpoint and generate only
"""
...
class WordLSTM(nn.Module):
    ...
    # x: (seq_len, batch) — token indices; hidden: (h, c) or None
    def forward(self, x, hidden=None):
        emb = self.embedding(x)               # (seq, batch, embed)
        out, hidden = self.lstm(emb, hidden)  # out: (seq, batch, hidden)
