...
I_{t,s} &= \sum_{h=1}^{n_h^I} w_{t,h}^I \cdot \operatorname{ReLU}\left( \mathbf{q}_{t,h}^I \cdot K_s^{\text{IComp}} \right),
...
$$
C_t^{\text{SprsComp}} = \left\{ C_s^{\text{Comp}} \;\bigg|\; I_{t,s} \in \text{Top-k}(I_{t,:}) \right\}. \tag{17}
$$
...
$$
\left[\mathbf{q}_{t,1};\, \mathbf{q}_{t,2};\, \dots;\, \mathbf{q}_{t,n_h}\right] = \mathbf{q}_t = \mathbf{c}_t^Q \cdot W^{UQ}, \tag{18}
$$
...
$$
o_{t,i} = \operatorname{CoreAttn}\left( \mathrm{query}=\mathbf{q}_{t,i},\, \mathrm{key}=C_t^{\mathrm{SprsComp}},\, \mathrm{value}=C_t^{\mathrm{SprsComp}} \right), \tag{19}
$$
...
$$
c × n_h × d = 512 × 128 × 7168 = 448M \tag{c}
$$

优化后，中间步骤计算量只是原来的1/7：

$$
c × \frac{n_h}{g} × d_g × g = 512 × 128 × 1024 = 64M \tag{d}
$$

最后一步的计算量是原来的1/4：

$$
g × d_g × d = 16 × 1024 × 7168 = 112M \tag{e}
$$
...
$$
\begin{align*}
S_{m'i:m'(i+1)-1} &= \mathrm{Softmax_{row}}\left(Z_{m'i:m'(i+1)-1} + B\right), \tag{22} \\
C_i^{\mathrm{Comp}} &= \sum_{j=m'i}^{m'(i+1)-1} S_j \odot C_j. \tag{23}
\end{align*}
$$
...
\left[\mathbf{q}_{t,1}; \mathbf{q}_{t,2}; \dots; \mathbf{q}_{t,n_h}\right] &= \mathbf{q}_t = \mathbf{c}_t^Q \cdot W^{UQ}, \tag{25}
\end{align*}
$$
...
$$
\mathbf{o}_{t,i} = \mathrm{CoreAttn}\left(\mathrm{query} = \mathbf{q}_{t,i},\, \mathrm{key} = \mathbf{C}^{\mathrm{Comp}},\, \mathrm{value} = \mathbf{C}^{\mathrm{Comp}}\right), \tag{26}
$$
